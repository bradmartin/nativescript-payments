...
export function init(): void {
  if (!_billingClient) {
    ...
    if (Application.android && Application.android.context) {
      _billingClient = com.android.billingclient.api.BillingClient.newBuilder(
        ...
        new com.android.billingclient.api.BillingClientStateListener({
          onBillingSetupFinished(resultCode: number): void {
            if (_billingClient) {
              if (
                resultCode ===
                com.android.billingclient.api.BillingClient.BillingResponse.OK
              ) {
                // Handle transactions already in queue
                const purchaseResult: PurchasesResult =
                  _billingClient.queryPurchases(
                    com.android.billingclient.api.BillingClient.SkuType.INAPP
                  ); // TODO
                _purchaseHandler(
                  purchaseResult.getResponseCode(),
                  purchaseResult.getPurchasesList()
                );
                _payments$.next({
                  context: Event.Context.CONNECTING_STORE,
                ...
                console.error(
                  new Error('Init failed with code: ' + resultCode)
                );
                _payments$.next({
                  context: Event.Context.CONNECTING_STORE,
